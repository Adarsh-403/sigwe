...
const ConvoyDashboardPage = ({ convoyId }) => {
  ...
  const handleAddDelivery = async () => {
    ...
    try {
      const res  = await fetch(
        (import.meta.env.VITE_API_URL || '') + `/api/convoys/${convoyId}/jobs/${activeJob.jobId}/deliveries`,
        {
          method:  'PUT',
  ...
  const executeConfirmAction = async () => {
    ...
    if (type === 'deliver') {
      try {
        const res  = await fetch(
          (import.meta.env.VITE_API_URL || '') + `/api/convoys/${convoyId}/jobs/${activeJob.jobId}/deliveries/${unitId}/deliver`,
          { method: 'PUT' },
        );
        const data = await res.json();
        if (data.success) fetchJobs();
      ...
      try {
        const res  = await fetch(
          (import.meta.env.VITE_API_URL || '') + `/api/convoys/${convoyId}/jobs/${activeJob.jobId}/deliveries/${unitId}/pay`,
          { method: 'PUT' },
        );
        const data = await res.json();
        if (data.success) fetchJobs();
